...
main(int ArgumentCount, char **Arguments)
{
    ...
    std::thread thread4 = std::thread(ProcessRows, FrameBuffer, RangePerThread*0, RangePerThread, Width, Height, Width, SamplesPerPixel, &Camera, Spheres, ARRAY_COUNT(Spheres));
    std::thread thread5 = std::thread(ProcessRows, FrameBuffer, RangePerThread*1, RangePerThread, Width, Height, Width, SamplesPerPixel, &Camera, Spheres, ARRAY_COUNT(Spheres));
    std::thread thread6 = std::thread(ProcessRows, FrameBuffer, RangePerThread*2, RangePerThread, Width, Height, Width, SamplesPerPixel, &Camera, Spheres, ARRAY_COUNT(Spheres));
    std::thread thread7 = std::thread(ProcessRows, FrameBuffer, RangePerThread*3, RangePerThread, Width, Height, Width, SamplesPerPixel, &Camera, Spheres, ARRAY_COUNT(Spheres));
